...
interface BulkTrimModalProps {
  onClose: () => void;
  onSave: (start?: string, end?: string) => void;
}

export function BulkTrimModal({ onClose, onSave }: BulkTrimModalProps) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-white/10 rounded-xl w-full max-w-sm shadow-2xl p-5 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-white font-medium">Bulk Trim</h3>
          <button 
            onClick={onClose}
        ...
        <div className="mb-4 text-xs text-white/50">
          <p>Apply these trim boundaries to ALL items in your batch queue.</p>
        </div>
